...
function mapOverallStatusLabel(status: string): string {
...
function mapBoolLabel(value: boolean): string {
...
export function SystemStatusPage() {
  ...
  if (query.isLoading) {
    return <LoadingBlock text="正在加载运行状态详情" />
  }
  ...
  if (!query.data || !latestStatus) {
    return <ApiErrorAlert message="未获取到运行状态详情" />
  }
  ...
  return (
    <div>
      <PageHeader
        title="运行状态详情"
        subtitle="查看初始化完成度、核心配置状态与最近运行状态"
      />
      ...
      <article className="card">
        ...
        <div className="dashboard-kv-list">
          <div>
            <span>系统主状态</span>
            <strong>{mapOverallStatusLabel(onboarding.overall_status)}</strong>
          </div>
          <div>
            <span>基础可运行</span>
            <strong>{mapBoolLabel(onboarding.basic_ready)}</strong>
          </div>
          <div>
            <span>完整可运行</span>
            <strong>{mapBoolLabel(onboarding.full_ready)}</strong>
          </div>
        </div>
      ...
      <article className="card" style={{ marginTop: '0.9rem' }}>
        ...
        <div className="dashboard-kv-list">
          ...
          <div>
            <span>测试通过 Provider</span>
            <strong>{testedProviders.length}</strong>
          </div>
          <div>
            <span>系统名称</span>
            <strong>{homeProfile.assistant_name || '家庭助手'}</strong>
          </div>
          <div>
            <span>日报生成时间</span>
      ...
      <article className="card" style={{ marginTop: '0.9rem' }}>
        ...
        <div className="dashboard-kv-list">
          ...
          <div>
            <span>最长未出现新视频时长</span>
            <strong>
              {videoPipelineHealth.maxMinutesSinceLastNewVideo !== null
                ? `${videoPipelineHealth.maxMinutesSinceLastNewVideo} 分钟`
                : '-'}
            </strong>
          </div>
        </div>
          ...
          <div className="summary-detail" style={{ marginTop: '0.8rem' }}>
            ...
            {alertSources.map((item) => (
              <article key={item.sourceId}>
                ...
                <p>
                  状态：{item.analysisState === 'paused' ? '已暂停' : item.analysisState === 'stopped' ? '已停止' : '识别中'}
                  {item.minutesSinceLastNewVideo !== null
                    ? `，距最近新视频 ${item.minutesSinceLastNewVideo} 分钟`
                    : ''}
                </p>
                <div className="row-actions">
                  <button
