...
def test_session_playback_returns_stream_and_manifest(pg_db: Session) -> None:
    with TemporaryDirectory() as tmp_dir, TemporaryDirectory() as video_root:
        ...
        try:
            ...
            playback_resp = get_session_playback(
                session_id=session.id, db=pg_db, locale="zh-CN", current_user=None
            )
...
def test_session_playback_marks_missing_segments_and_keeps_available_segment(
    pg_db: Session,
) -> None:
    with TemporaryDirectory() as cache_dir, TemporaryDirectory() as video_root:
        ...
        try:
            ...
            playback = get_session_playback(session.id, pg_db, "zh-CN", None)
            ...
            assert pg_db.get(VideoFile, missing.id).file_missing is True
            assert pg_db.get(VideoFile, missing.id).missing_at is not None
