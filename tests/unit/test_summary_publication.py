"""SQLite unit tests for the daily-summary publication use case (Todo 16).

These tests stand up an in-memory SQLite engine and exercise
:func:`publish_daily_summary` against the full model surface
(``daily_summary``, ``daily_summary_generation_attempt``, ``task_log``,
``outbox_event``). They verify the atomicity contract from ADR
``docs/adr/0011-transactional-outbox-and-task-lifecycle.md`` §1 — the
three writes (upsert summary + finalise attempt + enroll outbox) share
one transaction, so a ``rollback()`` leaves zero rows behind.

PostgreSQL-specific behaviour (partial unique indexes, transactional
visibility, FK ``SET NULL`` on ``task_log``, ``JSONB`` semantics) lives
in ``tests/integration/test_summary_publication_postgres.py``.
"""
...
def test_publish_succeeds_atomically_with_no_subscribers(db_session: Session) -> None:
    """The atomic write produces one summary + one attempt + zero outbox rows."""
    task_log = _new_task_log(db_session)
    attempt = _new_running_attempt(db_session, task_log_id=task_log.id, target=date(2026, 9, 2))

    outcome = publish_daily_summary(db_session, _cmd(db_session, attempt, task_log.id))
    db_session.commit()
    ...
    summary = (
        db_session.query(DailySummary).filter(DailySummary.summary_date == date(2026, 9, 2)).one()
    )
    ...
    assert db_session.query(TaskLog).count() == 1  # the attempt's bound log only


def test_publish_upserts_existing_summary_for_same_date(db_session: Session) -> None:
    """A second publish for the same date updates the row in place."""
    task_log = _new_task_log(db_session)
    attempt = _new_running_attempt(db_session, task_log_id=task_log.id, target=date(2026, 9, 2))

    first_outcome = publish_daily_summary(db_session, _cmd(db_session, attempt, task_log.id))
    db_session.commit()
    ...
    new_content = _sample_content(events_count=5, overall="已更新。", title="家庭日报 v2")
    ...
    rows = (
        db_session.query(DailySummary).filter(DailySummary.summary_date == date(2026, 9, 2)).all()
    )
    assert len(rows) == 1
...
def test_publish_enrolls_webhook_outbox_for_each_subscriber(db_session: Session) -> None:
    ...
    outcome = publish_daily_summary(
        db_session,
        _cmd(db_session, attempt, task_log.id, subscribers=[101, 102, 103]),
    )
    db_session.commit()
    ...
    task_logs = db_session.query(TaskLog).order_by(TaskLog.id).all()
    assert len(task_logs) == 4

    outbox_rows = db_session.query(OutboxEventRow).all()
...
def test_publish_does_not_emit_webhook_when_no_subscribers(db_session: Session) -> None:
    ...
    outcome = publish_daily_summary(
        db_session,
        _cmd(db_session, attempt, task_log.id, subscribers=[]),
    )
    db_session.commit()
    ...
    assert db_session.query(TaskLog).count() == 1


def test_publish_with_empty_events_count_still_succeeds(db_session: Session) -> None:
    """Empty-event day still upserts the summary and emits webhooks."""
    task_log = _new_task_log(db_session)
    attempt = _new_running_attempt(db_session, task_log_id=task_log.id, target=date(2026, 9, 2))
    content = _sample_content(events_count=0)
    ...
    assert db_session.query(OutboxEventRow).count() == 2
...
def test_publish_rejects_attempt_in_terminal_failure_state(db_session: Session) -> None:
    """An already-cancelled attempt raises and leaves no summary."""
    task_log = _new_task_log(db_session)
    attempt = _new_running_attempt(db_session, task_log_id=task_log.id, target=date(2026, 9, 2))
    attempt_repo = DailySummaryAttemptRepository(db_session)
    ...
    with pytest.raises(AttemptNotInValidStateError) as excinfo:
        publish_daily_summary(db_session, _cmd(db_session, attempt, task_log.id))
...
def test_publish_rejects_attempt_in_superseded_state(db_session: Session) -> None:
...
def test_publish_rejects_attempt_already_succeeded(db_session: Session) -> None:
...
def test_publish_rejects_missing_attempt(db_session: Session) -> None:
...
def test_publish_rollback_leaves_no_summary_no_attempt_no_outbox(
    ...
    """A rollback after publish leaves zero rows in all three tables.

    The fixture commits the ``TaskLog`` + ``attempt`` setup so the
    test can stand on a clean baseline; the publish itself must run in
    a fresh transaction that we then roll back. We open a second
    session against the same engine to verify the rolled-back writes
    are invisible.
    """
    task_log = _new_task_log(db_session)
    attempt = _new_running_attempt(db_session, task_log_id=task_log.id, target=date(2026, 9, 2))

    # The setup committed the baseline rows. Start a new SAVEPOINT so
    # the publish writes live in their own (doomed) tx and the
    # subsequent rollback discards them.
    publish_tx = db_session.begin_nested()
    try:
        publish_daily_summary(
            db_session,
            _cmd(db_session, attempt, task_log.id, subscribers=[301, 302]),
        )
    ...
    try:
        assert verify.query(DailySummary).count() == 0
        # The attempt row stays in the DB because it was committed in
        # the setup phase; the publish flow never touched it again
        # after the rollback.
        assert verify.query(DailySummaryGenerationAttempt).count() == 1
        # Only the attempt's bound log remains (committed in setup);
        # the per-subscriber webhook logs were rolled back.
        assert verify.query(TaskLog).count() == 1
...
def test_publish_dry_run_enrolls_no_webhooks_but_returns_payload(
    ...
    """``dry_run=True`` returns the outcome without enrolling webhook outbox rows."""
    task_log = _new_task_log(db_session)
    attempt = _new_running_attempt(db_session, task_log_id=task_log.id, target=date(2026, 9, 2))
...
def test_publish_value_error_when_task_log_id_mismatches_attempt(
    ...
    with pytest.raises(ValueError, match="task_log_id"):
        publish_daily_summary(
            db_session,
            _cmd(db_session, attempt, bogus_task_log_id),
        )
    db_session.rollback()
