...
def test_call_tool_get_daily_summary_success_and_log(pg_db: Session) -> None:
    pg_db.add(
        DailySummary(
    ...
    rows = pg_db.query(McpCallLog).all()
    assert len(rows) == 1
...
def test_call_tool_search_and_media_paths(pg_db: Session) -> None:
    source_id, session_id, event_id = _setup_source_session_event(pg_db)
...
def test_call_tool_ask_home_monitor_success(pg_db: Session, monkeypatch) -> None:
    _setup_source_session_event(pg_db)
...
def test_call_tool_invalid_argument_returns_tool_error(pg_db: Session) -> None:
    result = call_tool(
        db=pg_db,
        tool_name="get_daily_summary",
